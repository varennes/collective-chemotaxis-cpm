...
! Check the dependence of migration speed on the number of cells.
!
...
real(b8), allocatable :: firstpass(:),  MSD(:), MSDrun(:), xCOM(:,:), groupv(:,:), groupvRun(:,:)
real(b8) :: d, df, prob, r, uNew, uOld, w
real(b8) :: neMean, neMeanRun
real(b8) :: t0, tf

real(b8) :: nu, rx1, rx2, speciesR0
...
allocate(    groupv( tmax,2) )
allocate( groupvRun( tmax,2) )
...
do while( tMCS < tmax )
    ...
    elseif( sigma(b(1),b(2)) /= sigma(a(1),a(2)) )then
        ...
        if( check == N )then
            ...
            if( r < prob )then
                ! execute
                ! write(190,*) uNew-uOld, tELEM-1
                ! write(190,*) max(0.0, uNew-uOld - w ), tELEM-1
                sigma = sigmaTmp
                x     = xTmp
                uOld  = uNew
    ...
    if( mod( tELEM - 1, ne0) == 0 )then
        ...
        ! get cell group center of mass and displacement
        do i = 1, N
            call calcCellCOM( x(i,:,:),  cellCOM(i,:))
        ...
        groupv(tMCS,:) = groupv(tMCS,:) + xCOM(tMCS,:) - xCOM(tMCS-1,:)
...
groupvRun = groupvRun + groupv
! groupv(nRun,:) = groupv(nRun,:) / real(tcount-1)
...
! write(180,*) sum(groupv(:,1))/real(runTotal), sum(groupv(:,2))/real(runTotal)
groupvRun = groupvRun / real(runTotal)
do i = 1, tmax
    write(180,*) groupvRun(i,:), i-1
enddo
